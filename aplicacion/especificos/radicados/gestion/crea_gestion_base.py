...
def actualiza_radicado(radicado_id, clase_radicado, comentario_traslado, id_tarea):
    ...
    radicado   = sqalchemy_leer.leer_un_registro("radicados_entrada", radicado_id)
    atributos_ = radicado["atributos_"]
    atributos_["clase_radicado"] = clase_radicado
    nuevos_datos   = {
        "atributos_": atributos_
    }
    sqalchemy_modificar.modificar_un_registro("radicados_entrada", radicado_id, nuevos_datos)
...
def crea_registro_gestion(datos, archivos, id_tarea):
    ...
    # Crea archivos
    manejo_archivos.manejo("radicados_entrada", "insertar", {"id": radicado_id}, archivos, id_tarea )
    ...
    if (resuelto_inmediato == "SI"):        
        mensaje = "FINALIZA EN PRIMER CONTACTO LA GESTIÓN - " + atributos_["finalizado"]["finalizado_por_nombre"]
        datos_log = {
    ...
    notifica_gestion.invoca_enviar_notificado_gestion({"radicado_id": radicado_id, "responsable_id": responsable_id})
...
def crea_gestion_colaborativa(datos, peticion_id, id_tarea):
    ...
    # Radicado original 
    radicado       = sqalchemy_leer.leer_un_registro("radicados_entrada", peticion["origen_id"])       
    ...
    # Indexar estructuras
    indexar_datos.indexar_estructura("radicados_entrada", peticion["origen_id"], 90)
    indexar_datos.indexar_estructura("peticiones", gestion_id, 90)