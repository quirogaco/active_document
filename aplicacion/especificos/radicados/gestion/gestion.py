...
from librerias.datos.sql    import sqalchemy_modificar, sqalchemy_leer, sqalchemy_comunes, sqalchemy_insertar
...
def asigna_ventanilla(accion, datos={}, archivos=[], id_tarea=""):
    ...
    if es_pqrs == "SI":
        ...
        crea_gestion_base.crea_registro_gestion(datos_ventanilla, archivos, id_tarea)
...
def pqrs_gestion(estructura, accion, datos, tarea, archivos, id_tarea):    
    ...
    if es_ventanilla == "NO":
        datos["resuelto_inmediato"] = "NO" # Formulario asigna no tiene esta opcion
...
def asigna_pqrs(accion, datos={}, archivos=[], id_tarea=""):   
    ...
    if es_ventanilla == "SI":
        crea_gestion_base.actualiza_radicado(radicado_id, "VENTANILLA", comentario_traslado, id_tarea)        
    else:
        ...
        if (resuelto_inmediato == "SI"):
            ...
            datos_pqrs["atributos_"]["finalizado"] = {
                ...
                "finalizado_comentario": "RESUELTO EN PRIMER CONTACTO, " + datos_pqrs.get("comentario_traslado", "")
            } 
...
def asigna_reposicion(accion, datos={}, repone_id="", id_tarea=""):   
    ...
    relacion                 = sesion.query(GESTION_RELACIONES_CLASE).filter( GESTION_RELACIONES_CLASE.origen_id == repone_id ).first()
    if relacion != None:
        ...
        if peticion != None:
            ...
            sqalchemy_modificar.modificar_un_registro("peticiones", peticion.id, repone_datos)
            ...
            filtros       = [ [ "tipo_relacion", "=", "notificacion" ], [ "origen_id", "=", datos["id"] ] ]
            ...
            repuesto      = sqalchemy_leer.leer_un_registro("radicados_entrada", repone_id)
            datos_log = {
                ...
                "fuente_id"        : repone_id, 
                "accion"           : "REPONER",  
                "detalle"          : ( "REPOSICIÓN CON RADICADO #: " + datos["nro_radicado"]),
                "estado"           : "ACTIVADO",  
            ...
            datos_log = {
                ...
                "fuente_id"        : datos["id"], 
                "accion"           : "REPONER",  
                "detalle"          : ( "REPONE RADICADO CON #: " + repuesto["nro_radicado"]),
                "estado"           : "ACTIVADO",  
