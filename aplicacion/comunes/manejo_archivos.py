...
def manejo(estructura, accion, datos, archivos, id_tarea, tipo_relacion = "anexos", cubeta = "contenedor.general"):
    secuencia = [
        {
    ...
    llamados.ejecutar_secuencia(secuencia=secuencia, ubicacion="comunes manejo archivos")  
...
def buca_anexo_especifico_id(origen_id, tipo_anexo="notificacion"):
    ...
    filtros = [ [ "origen_id", "=", origen_id ], [ "tipo_relacion", "=", tipo_anexo] ]
    ...
    if len(relaciones) > 0:
        ...
        archivos = sqalchemy_filtrar.filtrarOrdena(estructura="archivos_electronicos", filtros=filtros, ordenamientos=[])
...
def recupera_anexo_especifico_radicado(nro_radicado, tipo_radicado="ENTRADA", tipo_anexo="notificacion"):
    nombre_archivo = ""
    if tipo_radicado == "ENTRADA":
        ...
        if len(radicados) > 0:
            nombre_archivo = recupera_anexo_especifico_id(radicados[0]["id"], tipo_anexo)
