...
function CustomContextPadProvider(
    ...
    this._bpmnFactory    = elementFactory._bpmnFactory
...
CustomContextPadProvider.prototype.getContextPadEntries = function (element) {
    ...
    //console.log("businessObject-1:", businessObject)
     ...
     */
    // Adiciona elemento a contexto
    function appendAction(type, className, title, options) {
        //console.log("appendAction:", type)
    ...
    function createTask(accion_tipo) {      
      return function(event, origen) {        
        const businessObject = bpmnFactory.create('bpmn:UserTask');

        contador  += 1 
        let id     = accion_tipo.replace(" ", "_") + "_" + contador
        let nombre = accion_tipo + "_" + contador        
        businessObject.id          = id
        businessObject.name        = nombre
        businessObject.accion_tipo = accion_tipo
        console.log("bpmn:UserTask", businessObject)

        const destino = elementFactory.createShape({
            type: 'bpmn:UserTask',
            businessObject: businessObject
        });

        autoPlace.append(origen, destino);

        //create.start(event, shape);
      };
    }

    function appendUserTask(event, origen) {        
        if (autoPlace) {    
            //console.log("elementFactory.createShape:", elementFactory.createShape)      
            const destino = elementFactory.createShape({ type: 'bpmn:UserTask' });
            origen.businessObject.id   = "x01"
            origen.businessObject.name = "mmmmmmmm----mmmmmmm"
            //console.log("USER origen:", origen, origen.businessObject)
            destino.businessObject.id   = "y01"
            destino.businessObject.name = "nnnnnnnnn-nnnn"
            //console.log("USER destino:", destino, destino.businessObject)
    ...
    function appendUserTaskStart(event) {
        //console.log("appendUserTaskStart:", event)
    ...
    if (!is(businessObject, 'bpmn:EndEvent')) {
        assign(actions, {           
            'append.service-task1': {
                group: 'model1',
                //imageUrl: dataURL,
                className: 'aprobar',
                title: translate('Visto Bueno'),
                action: {
                    click: createTask("Visto bueno"),
                    //dragstart: appendUserTaskStart
                }
            },

            'append.service-task2': {
                group: 'model1',
                className: 'asignar',
                title: translate('Asigna Responsable'),
                action: {
                    click: createTask("Asignacion"),
                    //dragstart: appendUserTaskStart
                }
            },

            'append.service-task3': {
                group: 'model1',
                className: 'radicar',
                title: translate('Radicar'),
                action: {
                    click: createTask("Radicacion"),
                    //dragstart: appendUserTaskStart
                }
            },
            ...
            'append.service-task': {
                group: 'model1',
                className: 'gestionar',
                title: translate('Gestion usuario'),
                action: {
                    click: createTask("Gestion"),
                    //dragstart: appendUserTaskStart
                }
            },
        })
    ...
    //***** PUERTA EXCLUSIVA **********/
    //*********************************/
    function appendExclusiveGateway(event, origen) {        
        //console.log("ExclusiveGateway autoPlace:", autoPlace)
        if (autoPlace) {    
            //console.log("elementFactory.createShape:", elementFactory.createShape)      
            const destino = elementFactory.createShape({ type: 'bpmn:ExclusiveGateway' });
            //console.log("ExclusiveGateway origen:", origen, origen.businessObject)
            destino.businessObject.actividaOrigen = origen.businessObject
            //console.log("ExclusiveGateway destino:", destino, destino.businessObject)
            var shape = autoPlace.append(origen, destino); 
            //console.log("ExclusiveGateway shape:", shape)
        } else {
            appendUserTaskStart(event, origen);
        }
    }
    ...
    if (!is(businessObject, 'bpmn:EndEvent')) {
        assign(actions, {
            'append.gateway': {
                ...
                title: translate('Decisión de ruta'),
                action: {
